		-- Задача 1. Получение статистики по заработной плате
...
-- является ошибкой данных, а максимальная достигает 497500 рублей.
		...
		-- Задача 2. Количество вакансий по регионам
...
-- количество вакансий — это указывает на развитый рынок труда для аналитиков данных в этих регионах.
		...
		-- Задача 3. Количество вакансий по компаниям
...
-- покупать на маркетплейсах, но и проанализировать, что другие покупают.
		...
		-- Задача 4. Количество вакансий по типу занятости
SELECT employment,
       COUNT(*) AS num_vacancies
FROM public.parcing_table 
GROUP BY employment 
...
-- долго в них участвовать.
		...
		-- Задача 5. Количество вакансий по графику работы
SELECT schedule,
       COUNT(*) AS num_vacancies 
FROM public.parcing_table 
GROUP BY schedule 
...
-- и изменения в предпочтениях работников.
		...
		-- Задача 6. Выявление грейда требуемых специалистов по опыту
SELECT experience,
       COUNT(*) AS num_vacancies
FROM public.parcing_table 
GROUP BY experience 
...
-- Однако важно понимать, что Senior-аналитиков могут чаще искать по другим каналам.
		...
		-- Задача 6. Определение доли грейдов среди вакансий аналитиков
...
-- но это совсем не так — тут уже надо понимать особенности рынка.   
		...
		-- Задача 7. Определение типичного места работы для аналитиков по различным параметрам
SELECT employer,
       COUNT(*) AS num_vacancies,
...
-- Если вам подходят такие условия, то потенциальных работодателей для вас будет много.
		...
		-- Задача 8. Частота упоминания hard и soft skills (пример для key_skills_1) (Аналогично для остальных столбцов key_skills и soft_skills)
SELECT key_skills_1,
